...
function handleSelectOfficer(officer) {
    if (confirm(`Tugaskan tiket ${currentTicketId} kepada ${officer.name}?`)) {
        // Simulasi POST ke PHP (Assign Officer)
        alert(`Petugas ${officer.name} berhasil ditugaskan! Status tiket diperbarui.`);
        closeOfficerModal();
...
function renderOfficerSelectionModal(ticketId, currentOfficerName) {
    ...
    mockOfficers.forEach((officer) => {
        ...
        const cardClasses = `w-full p-4 border-2 rounded-xl cursor-pointer transition-all text-left ${
            isSelected 
        ...
        const actionAttribute = isAvailable || isSelected ? `onclick='handleSelectOfficer(${JSON.stringify(officer)})'` : `disabled`;

        officerListHtml += `
            <button key="${officer.id}" ${actionAttribute} class="${cardClasses}">
    ...
    const modalHtml = `
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onclick="closeOfficerModal()">
            <div class="bg-white rounded-xl max-w-2xl w-full max-h-[80vh] overflow-hidden" onclick="(e) => e.stopPropagation()">
...
const originalCloseModal = closeOfficerModal;
closeOfficerModal = () => {

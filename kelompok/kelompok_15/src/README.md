...
```bash
#!/usr/bin/env bash
# KELASONLINE - quick install script (PHP Native) instalasi
# Sesuaikan variabel di bawah sebelum menjalankan
PROJECT_DIR="TUBES_PRK_PEMWEB_2025_KELOMPOK-15"   # nama folder project
WEB_ROOT="/var/www/html"                         # ubah jika menggunakan path lain (XAMPP: /opt/lampp/htdocs)
DB_NAME="kelasonline"
DB_USER="root"
DB_PASS=""                                       # isi jika MySQL memiliki password
SQL_SCHEMA_FILE="database/schema.sql"            # relatif terhadap root project
SEED_FILE="database/seed.sql"                    # opsional (biar ada data contoh)

# Fungsi helper
print() { echo -e "\n==> $1"; }

# 1) Pastikan paket dasar ada (php, mysql, apache)
print "Cek apakah mysql client tersedia..."
if ! command -v mysql >/dev/null 2>&1; then
  echo "mysql client tidak ditemukan. Install terlebih dahulu (contoh: sudo apt install mysql-client)"; exit 1
fi

# 2) Clone repo (jika belum ada)
if [ ! -d "$PROJECT_DIR" ]; then
  print "Clone repository (jika belum):"
  git clone https://github.com/Esbeka/TUBES_PRK_PEMWEB_2025_KELOMPOK-15.git "$PROJECT_DIR" || { echo "Gagal clone repo"; exit 1; }
else
  print "Folder '$PROJECT_DIR' sudah ada — melewati clone."
fi

# 3) Copy project ke web root (memerlukan sudo jika /var/www/html)
print "Menyalin project ke web root ($WEB_ROOT)..."
sudo rsync -a --delete "$PROJECT_DIR/" "$WEB_ROOT/$PROJECT_DIR/" || { echo "Gagal menyalin ke $WEB_ROOT"; exit 1; }

PROJECT_PATH="$WEB_ROOT/$PROJECT_DIR"

# 4) Set permission untuk folder uploads
print "Mengatur permission pada folder uploads..."
sudo mkdir -p "$PROJECT_PATH/uploads" 
sudo chown -R "$USER":www-data "$PROJECT_PATH/uploads" || true
sudo chmod -R 775 "$PROJECT_PATH/uploads" || true

# 5) Buat database & import schema
print "Membuat database dan import schema..."
...
echo "CREATE DATABASE IF NOT EXISTS \`${DB_NAME}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" | $MYSQL_CMD || { echo "Gagal membuat database. Cek credential MySQL."; exit 1; }

# Import schema
if [ -f "$PROJECT_PATH/$SQL_SCHEMA_FILE" ]; then
  $MYSQL_CMD "$DB_NAME" < "$PROJECT_PATH/$SQL_SCHEMA_FILE" || { echo "Gagal import schema.sql"; exit 1; }
  print "Schema di-import."
else
  echo "File schema tidak ditemukan: $PROJECT_PATH/$SQL_SCHEMA_FILE"
fi

# (Opsional) Import seed data
if [ -f "$PROJECT_PATH/$SEED_FILE" ]; then
  print "Mengimport seed data (opsional)..."
  $MYSQL_CMD "$DB_NAME" < "$PROJECT_PATH/$SEED_FILE" || echo "Gagal import seed (lanjutkan)."
fi

# 6) Update konfigurasi database PHP (config/database.php)
CONFIG_FILE="$PROJECT_PATH/config/database.php"
if [ -f "$CONFIG_FILE" ]; then
  print "Mengupdate $CONFIG_FILE dengan kredensial DB yang diset..."
  # backup dulu
  sudo cp "$CONFIG_FILE" "${CONFIG_FILE}.bak"
  # replace values (asumsi format sederhana)
  sudo sed -i "s/\\\$host *= *'.*';/\\\$host = 'localhost';/" "$CONFIG_FILE"
  sudo sed -i "s/\\\$dbname *= *'.*';/\\\$dbname = '${DB_NAME}';/" "$CONFIG_FILE"
  sudo sed -i "s/\\\$username *= *'.*';/\\\$username = '${DB_USER}';/" "$CONFIG_FILE"
  # handle empty or non-empty password (escape slashes)
  ESC_PASS=$(printf '%s\n' "$DB_PASS" | sed -e 's/[&/]/\\&/g')
  sudo sed -i "s/\\\$password *= *'.*';/\\\$password = '${ESC_PASS}';/" "$CONFIG_FILE"
else
  echo "Peringatan: file config/database.php tidak ditemukan di $CONFIG_FILE. Edit manual jika perlu."
fi

# 7) Jika XAMPP di Linux tersedia, beri instruksi start
if [ -x "/opt/lampp/lampp" ]; then
  print "XAMPP (lampp) terdeteksi. Untuk memulai server, jalankan:"
  echo "  sudo /opt/lampp/lampp start"
else
  print "Pastikan Apache & MySQL berjalan. Contoh (Ubuntu):"
  echo "  sudo systemctl start apache2"
  echo "  sudo systemctl start mysql"
fi

# 8) Selesai & URL akses
print "Selesai. Akses aplikasi di browser:"
echo "  http://localhost/$PROJECT_DIR/pages/index.html"
echo
echo "Catatan:"
echo " - Jika menggunakan XAMPP on Windows, salin folder project ke C:\\xampp\\htdocs\\"
echo " - Jika MySQL menggunakan password, set DB_PASS di atas sebelum menjalankan script"
echo " - File konfigurasi backup: $CONFIG_FILE.bak (jika ada)"
```

...
if (isset($_POST['action']) && $_POST['action'] == 'delete') {
    ...
    try {
        ...
        if($d) {
            if($d['file_path'] && file_exists("../uploads/tugas/".$d['file_path'])) {
                unlink("../uploads/tugas/".$d['file_path']);
            }
            $stmtMhs = $pdo->prepare("SELECT file_path FROM submit_tugas WHERE tugas_id = ?");
            $stmtMhs->execute([$id]);
            ...
            $pdo->prepare("DELETE FROM submit_tugas WHERE tugas_id = ?")->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
        ...
        $file = "soal_".uniqid().".".$ext;
        if (!file_exists('../uploads/tugas/')) mkdir('../uploads/tugas/', 0777, true);
        move_uploaded_file($_FILES['file']['tmp_name'], "../uploads/tugas/".$file);
    }

    try {
        $sql = "INSERT INTO tugas (judul, mata_kuliah_id, deskripsi, deadline, file_path, created_by) VALUES (?,?,?,?,?,?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$judul, $mk, $desc, $dl, $file, $dosen_id]);
